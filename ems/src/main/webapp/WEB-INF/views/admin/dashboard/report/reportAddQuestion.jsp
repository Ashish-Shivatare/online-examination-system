...
<body>
	<div class="container">
		<div class="h1">Update Question</div>
					...
					<label>Question Type</label> <select class="form-control"
						id="questionType" name="questionType">	
							<c:forEach var="type" items="${questionTypeList}">
								<option value="${type.questionType}"><c:out value="${type.questionType}"></c:out></option>
							</c:forEach>
					</select>
				</div>
				<div class="form-group">
					...
					<div class="form-check  form-check-inline">
						<p id="correctAns">
						<c:forEach var="correctOption"  begin="1" end="${fn:length(que.questionOption)}" varStatus="answerCount">
							<input class="form-check-input" type="${que.answerButton}" name="correctAnswer"
								id="correctAnswer${correctOption}" value="${answerCount.count}"> <label
								class="form-check-label"><c:out value="${correctOption}" /></label>
						</c:forEach>
						</p>
					</div>
				</div>
